...
public class Searcher
{
    ...
    public async Task GetTables(BindingSource LawActSource, BindingSource LawExecSource)
    {
        ...
        if (Search_IP.Length > 0 || Search_ID.Length > 0)
        {
            //ispol proizvodstvo
            if (Search_IP.Length > 0)
            {
                data2 = data2.Where(x => EF.Functions.Like(x.fssp_doc_num, $"{Search_IP}%"));
            }
            //ispol doc
            if (Search_ID.Length > 0)
            {
                data2 = data2.Where(x => EF.Functions.Like(x.court_doc_num, $"{Search_ID}%"));
            }
            if (data2.Count() > 0)
            {
        ...
        if(fio != null || contract != null || exec_number != null || law_id != null)
        {
            //FIO
            if (fio != null)
            {
            ...
            if (data1.Count() > 0)
            {
                var select1 = data1.Select(x => new ActDataModel()
                {
                    ...
                    LawCourt = x.LawCourt != null ? new()
                    {
                        id = x.LawCourt.id,
                        name = x.LawCourt.name,
                        address = x.LawCourt.address
                    } : null
                });
